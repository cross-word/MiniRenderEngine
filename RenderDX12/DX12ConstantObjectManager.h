...
class DX12ConstantObjectManager
{
public:
	DX12ConstantObjectManager();
	~DX12ConstantObjectManager();

	virtual void InitialzieUploadBuffer(ID3D12Device* device, ID3D12GraphicsCommandList* cmdList, UINT numConstants);
	virtual void InitializeSRV(ID3D12Device* device, const D3D12_CPU_DESCRIPTOR_HANDLE* cpuHandle, UINT numConstants);
...
class DX12MaterialConstantManager : public DX12ConstantObjectManager
{ 
public:
	void InitialzieUploadBuffer(ID3D12Device* device, ID3D12GraphicsCommandList* cmdList, UINT byteSize) override;
	void InitializeSRV(ID3D12Device* device, const D3D12_CPU_DESCRIPTOR_HANDLE* cpuHandle, UINT numConstants) override;
...
class DX12ObjectConstantManager : public DX12ConstantObjectManager
{
public:
